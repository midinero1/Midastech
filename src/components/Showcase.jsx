  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-[1.25rem] bg-[#1c1813] px-3 pb-3.5 pt-4">
        ...
        <p className="mt-1.5 font-display text-[0.72rem] leading-none text-canvas">{c.title}</p>
        <p className="mt-1 text-[0.44rem] font-semibold uppercase tracking-[0.18em] text-gilt">{c.ready}</p>

        <div className="mt-3 flex flex-col gap-1.5">
          {c.menu.map(([item, price], i) => (
            <div
              key={item}
  ...
  return (
    <div className="relative" aria-hidden="true">
      ...
      <div className="drift relative pl-8 sm:pl-14 lg:pl-10">
        <Browser c={c} />
        <Phone c={c} className="absolute -bottom-10 -left-2 sm:-bottom-12 sm:-left-4 lg:-bottom-14 lg:-left-6" />
      </div>
    </div>
  )
}
